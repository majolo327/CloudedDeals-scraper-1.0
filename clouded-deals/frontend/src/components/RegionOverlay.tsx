...
export function RegionOverlay({ stateCode, zip, onBrowseVegas, onEmailSubmit }: RegionOverlayProps) {
  ...
  if (isNorthernNV) {
    return (
      <div
        className="fixed inset-0 z-[100] overflow-y-auto animate-overlay-in overscroll-contain"
        onClick={handleDismiss}
      >
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm" />
        <div className="relative min-h-full flex items-center justify-center p-4">
          <div
            className="relative w-full max-w-md glass-strong frost rounded-2xl p-6 sm:p-8 text-center animate-card-slide-up"
            onClick={(e) => e.stopPropagation()}
          >
            <button
              onClick={handleDismiss}
              className="absolute top-3 right-3 w-9 h-9 min-w-[44px] min-h-[44px] rounded-full flex items-center justify-center text-slate-400 hover:text-white hover:bg-white/10 transition-all"
            >
              <X className="w-5 h-5" />
            </button>
    ...
    >
      ...
      <div className="relative min-h-full flex items-center justify-center p-4">
        ...
        >
          ...
          {/* Logo */}
          <h1 className="text-lg font-bold tracking-tight mb-5">
            Clouded<span className="text-purple-400">Deals</span>
          </h1>
            ...
            <div className="mb-5 space-y-3">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  placeholder={contactMethod === 'phone' ? '(702) 555-1234' : "Drop your email"}
                  className="flex-1 min-w-0 px-3 py-3 min-h-[48px] rounded-xl bg-white/[0.06] border border-white/[0.08] text-white text-sm placeholder:text-slate-600 focus:outline-none focus:border-purple-500/40 transition-colors"
                />
                <button
                  onClick={handleContactSubmit}
                  className="px-4 py-3 min-h-[48px] rounded-xl bg-purple-600 hover:bg-purple-500 text-white font-semibold transition-colors text-sm shrink-0"
                >
                  Notify Me
                </button>
              </div>
