/**
 * Resolve a US zip code to a state abbreviation using the ZIP3 prefix.
 * This is ~99% accurate for state-level resolution with zero API calls.
 *
 * Data source: USPS ZIP code prefix allocation table.
 */
...
/**
 * Determine if an NV zip code is in southern NV (Las Vegas area) vs northern NV (Reno).
 * 889xx-891xx = Las Vegas / Southern NV
 * 893xx-898xx = Reno / Northern NV
 */
export function getNevadaRegion(zip: string): 'southern-nv' | 'northern-nv' {
  const cleaned = zip.replace(/\D/g, '').slice(0, 3);
  const prefix = parseInt(cleaned, 10);
  if (prefix >= 893) return 'northern-nv';
  return 'southern-nv';
}
