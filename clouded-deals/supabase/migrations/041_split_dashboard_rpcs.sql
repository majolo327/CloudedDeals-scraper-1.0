-- =========================================================================
-- 041_split_dashboard_rpcs.sql — Split dashboard into parallel RPCs
-- =========================================================================
-- The monolithic get_dashboard_metrics() times out on Supabase (8s limit).
-- Split into 4 focused RPCs that the frontend calls in parallel.
-- Each has SET statement_timeout = '30s' as a safety net.
-- Safe to run multiple times (CREATE OR REPLACE).
-- =========================================================================

-- =========================================================================
-- 1. get_dashboard_growth(window_days)
-- =========================================================================
...
BEGIN
  ...
  WHERE created_at >= CURRENT_DATE AND anon_id IS NOT NULL;

  -- MAU
  SELECT COALESCE(COUNT(DISTINCT anon_id), 0) INTO v_mau
  FROM analytics_events
  WHERE created_at >= NOW() - INTERVAL '30 days' AND anon_id IS NOT NULL;

  -- Total unique users
  SELECT COALESCE(COUNT(DISTINCT anon_id), 0) INTO v_total_users
  FROM analytics_events WHERE anon_id IS NOT NULL;
  ...
  -- Daily unique visitors sparkline
...
-- =========================================================================
-- 2. get_dashboard_retention(window_days)
-- =========================================================================
...
BEGIN
  ...
  cohort_retention AS (
    SELECT
      cu.cohort_monday, cu.anon_id, cu.first_day,
...
BEGIN
  ...
  -- Total shares (lightweight — included here to avoid a 5th RPC)
