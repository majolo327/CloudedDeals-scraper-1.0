-- =========================================================================
-- 040_dashboard_metrics.sql — Unified dashboard metrics RPC
-- =========================================================================
-- Single RPC returning all metrics needed for the admin dashboard.
-- Fixes save rate calculation (user-based, not event-based).
-- Adds weekly cohort format (Week 0-4) for investor-standard reporting.
...
BEGIN
  ...
  -- NOT total save events / sessions (which produced 380%)
  WITH window_data AS (
    SELECT
  ...
  -- "First session" = first calendar day of activity
  WITH new_users AS (
    SELECT anon_id, MIN(created_at::date) AS first_day
    FROM analytics_events
    WHERE anon_id IS NOT NULL
  ...
  -- Return rate: sessions from users with prior visits / total sessions
  -- Proxy: users active on multiple distinct days / total active users
  WITH user_days AS (
  ...
  FROM (
    SELECT
      ...
      100 AS week0,
      -- Only show week N if that week has fully elapsed
  ...
  SELECT ROUND(
    AVG(
      ...
      THEN COALESCE(array_length(sites_scraped, 1), 0)::numeric
        / (COALESCE(array_length(sites_scraped, 1), 0) + COALESCE(array_length(sites_failed, 1), 0))
        * 100
      ELSE 100
      END
    ), 0
...
COMMENT ON FUNCTION get_dashboard_metrics IS
  'Unified dashboard metrics RPC. Returns growth, retention, cohorts (weekly), pipeline, viral, and coverage data in a single call. Save rate is user-based (not event-based).';
